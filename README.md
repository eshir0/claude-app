...
모듈을 노출해줄 뿐, 모듈 자체를 만들어주지는 않는다. `proxmox` 모듈이 이 패턴이 실제로
두 번째 모듈에도 그대로 적용되는지 확인해준 사례다.

## 테스트

```bash
npm run test:unit          # 순수 로직/검증 스키마 — DB·인증·네트워크 불필요, 50개
...
- **HTTP/서버 레벨로 실행해 확인**: 유닛 테스트 50/50, 프로덕션 standalone 서버를 실제로
