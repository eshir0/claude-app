...
| **접속 기록**<br>`/access-log` | 이 앱을 포함한 여러 자체 호스팅 서버로 들어온 접속을 한 곳에서: IP, 어느 서버로 들어왔는지, 몇 번, 언제, (선택) 대략 어느 나라/도시인지 | 각 서버가 `scripts/ip-log-agent.mjs`를 자기 앞단에 띄워 실제 접속을 로컬에서 캡처하고 이 앱의 ingest API로 보고 — 자세한 구성은 아래 "접속 기록" 절 참고 |
